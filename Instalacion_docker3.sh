...
sudo apt update && sudo apt full-upgrade -y && sudo apt autoremove -y > "$Logslocal" 2>&1
echo "-----------------------------------------------------------------------" >> "$Logslocal"
...
Actualizar() {
  ssh "$user@$ip" << EOF
  set -e
  mkdir -p /home/david/TFG
  Logsremoto=/home/david/TFG/Logs.txt
  apt update >> "\$Logsremoto" 2>&1
  apt install -y apt-transport-https ca-certificates curl gnupg lsb-release >> "\$Logsremoto" 2>&1
...
docker_borrar() {
  ssh "$user@$ip" << EOF
    ...
    apt-get purge -y docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin docker docker-engine docker.io containerd runc >> "\$Logsremoto" 2>&1
...
EOF
  ...
  ssh root@"$ip" << EOF
    ...
    docker pull pihole/pihole:latest >> "$Logsremoto" 2>&1
    ...
    until docker exec pihole pihole status &>/dev/null; do slcd eep 5; done
...
echo ""
echo ""
echo "puede acceder a pihole desde http://$ip:$puerto/admin/"
sleep 5
firefox "http://$ip:$puerto/admin"
echo "-----------------------------------------------------------------------"
echo "-----------------------------------------------------------------------"
sleep 5
echo ""
...
EOF
  ...
  ssh root@"$ip" bash -s << EOF
    ...
    docker volume inspect gitlab_config  >> "$Logsremoto" 2>&1 || docker volume create gitlab_config >> "$Logsremoto" 2>&1
    docker volume inspect gitlab_logs  >> "$Logsremoto" 2>&1 || docker volume create gitlab_logs >> "$Logsremoto" 2>&1
    docker volume inspect gitlab_data  >> "$Logsremoto" 2>&1 || docker volume create gitlab_data >> "$Logsremoto" 2>&1

    docker pull gitlab/gitlab-ce:latest  >> "$Logsremoto" 2>&1

    docker run -d --name gitlab --network gitlab-net \
    ...
    sleep 60
    echo "puede iniciar sesion como usuario : root "
    echo "Contraseña inicial de root para GitLab:"
    docker run --rm -v gitlab_config:/config alpine cat /config/initial_root_password | grep "Password" | awk '{print$2}'
 
EOF
echo "-----------------------------------------------------------------------"
echo ""
echo ""
echo "puede acceder a Gitlab desde http://$ip:$puerto"
sleep 5
...
echo "Saliendo del programa..."
  scp "$user@$ip:/home/david/TFG/Logs.txt" ./Logs_remoto.txt
  echo "Archivo de logs copiado a ./Logs_remoto.txt"
  exit 0 
}
...
echo "-------------------------------------------------------------------------------------------"
echo "-------------------------------------------------------------------------------------------"
echo "    Se ha finalizado la Instalación de docker y sus servicios, saliendo del programa"
echo "-------------------------------------------------------------------------------------------"
echo "-------------------------------------------------------------------------------------------"